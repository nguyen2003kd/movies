...
const Movie = () => {
  ...
  const{searchMovies,searchEdit,clinkSearch}=useStoremovie((state)=>state)
  useEffect(() => {
    if (!API_KEY || !BASE_URL) {
        ...
        <div className="bg-[#0F0F0F] static">
          <main className="flex-1">
            ...
            <div className="bg-black-main px-4 md:px-8 py-8 xl:p-16">
              ...
              <Search urlmovies={`${BASE_URL}/search/movie?api_key=${API_KEY}`} searchMovies={searchMovies} searchEdit={searchEdit} clinkSearch={clinkSearch}/>
              <div className="flex flex-wrap -mx-2 mt-16">
                {movies.map((item,index)=>(
                  <div className="px-2 w-1/2 md:w-1/4 lg:w-1/6 mb-8" key={index} >
                    <a href={`/movie/${item.id}`} className="hover:cursor-pointer group/container z-10">
                  <div
                  style={{
                    backgroundImage:
