...
const Movie = () => {
  ...
  const movies=useStoreTV((state)=>state.moviesTV)
  const fetchMovie=useStoreTV((state)=>state.fetchMovie)
  const viewMore=useStoreTV((stare)=>stare.viewMore)
  const setUrl=useStoreTV((state)=>state.setUrl)
  const{searchMovies,searchEdit,clinkSearch}=useStoreTV((state)=>state)
  useEffect(() => {
    if (!API_KEY || !BASE_URL) {
      console.error("API_KEY hoặc BASE_URL không được định nghĩa trong .env");
      return;
    }
    setUrl(`${BASE_URL}/trending/tv/day?api_key=${API_KEY}`)
    fetchMovie(`${BASE_URL}/trending/tv/day?api_key=${API_KEY}`)
        ...
        <div className="bg-[#0F0F0F] min-h-screen  static">
          <main className="flex-1">
            ...
            <div className="bg-black-main px-4 md:px-8 py-8 xl:p-16">
              ...
              <Search urlmovies={`${BASE_URL}/search/tv?api_key=${API_KEY}`} searchMovies={searchMovies} searchEdit={searchEdit} clinkSearch={clinkSearch}/>
              <div className="flex flex-wrap -mx-2 mt-16">
                {movies.map((item,index)=>(
                  ...
                  className="relative w-full h-72 2xl:h-80 rounded-3xl bg-center bg-no-repeat bg-cover group/poster after:absolute after:top-0 after:right-0 after:bottom-0 after:left-0 after:rounded-3xl hover:after:bg-black/60 after:transition after:ease-in-out after:duration-300"
                >
                  <Iconyst/>
                </div>
